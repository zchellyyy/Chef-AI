...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE recipes SET menu_name = ?, category = ?, featured_image = ? WHERE id = ?");
    $stmt->bind_param("sssi", $menu_name, $category, $image_path, $recipe_id);
    $stmt->execute();
    $stmt->close();

    $conn->query("DELETE FROM recipe_ingredients WHERE recipe_id = $recipe_id");

    $desc_arr = $_POST['ingredient_description'];
    $cost_arr = $_POST['ingredient_cost'];
    $mass_arr = $_POST['ingredient_mass'];
    $unit_arr = $_POST['ingredient_unit'];

    $stmt = $conn->prepare("INSERT INTO recipe_ingredients (recipe_id, description, cost, mass, unit) VALUES (?, ?, ?, ?, ?)");
    for ($i = 0; $i < count($desc_arr); $i++) {
        $desc = $desc_arr[$i];
        $cost = floatval($cost_arr[$i]);
        $mass = floatval($mass_arr[$i]);
        $unit = $unit_arr[$i];
        $stmt->bind_param("isdss", $recipe_id, $desc, $cost, $mass, $unit);
        $stmt->execute();
    }
    $stmt->close();

    echo "<script>alert('Recipe updated successfully!'); window.location.href = 'save_recipe.php?id=$recipe_id';</script>";
    exit;
}
...
<div class="container">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="existing_image" value="<?= $recipe['featured_image'] ?>">

...
// ======================================================================
// ENHANCED AI VERIFICATION FUNCTION FOR INGREDIENTS ONLY
// ======================================================================
/**
...
// ======================================================================
// AI VERIFICATION FOR IMAGE GENERATION (dish names allowed)
// ======================================================================
...
2. also accept unique ingredients like safflawor, annatoseed etc.
...
/* FIXED: Completely remove overlay sidebar, make static sidebar work as drawer */
@media (max-width: 768px) {
    .sidebar {
        ...
        background: linear-gradient(180deg, rgba(79,56,36,0.98), rgba(106,76,50,0.98)) !important;
    ...
    /* Ensure chat header is above everything */
    .chat-header {
        z-index: 1000;
        position: relative;
    }
    ...
    /* Make sure history button is visible */
    .history-btn {
...
.history-btn:hover {
  background: rgba(107, 79, 51, 0.1);
  transform: scale(1.1);
}

/* Generate Recipe Button - HIDDEN (moved to plus dropdown) */
.generate-recipe-btn {
  display: none;
}
...
/* FIXED: Plus button and dropdown - BETTER POSITIONING */
...
/* FIXED: Recipe Modal Responsive Height */
...
/* MOBILE FULL-SCREEN CHATBOX FIXES */
@media (max-width: 768px) {
  body {
    margin: 0;
    padding: 0;
  ...
  .chat-header {
    ...
    padding: 12px 15px;
    border-radius: 0;
    background: linear-gradient(90deg, rgba(255,191,38,0.12), rgba(111,45,134,0.08));
    border-bottom: 1px solid rgba(107, 79, 51, 0.1);
    flex-shrink: 0;
  }
  ...
  /* Hide intro container on mobile when not needed */
  .intro-container {
    margin-top: 30vh;
    padding: 20px;
  }

  /* Quick tags adjustments */
  .quick-tags {
    margin-bottom: 10px;
    justify-content: flex-start;
  }

  /* Recipe modal full screen */
  .recipe-modal-content {
    width: 100vw;
    height: 100vh;
    margin: 0;
...
/* PREVENT BACKGROUND BODY FROM SHOWING */
@media (max-width: 768px) {
  body::before {
    content: '';
    position: fixed;
...
@media (max-width: 768px) {
  ...
  /* Adjust chat card height for mobile */
  .chat-card {
  ...
  <div class="recipe-modal-content">
    ...
    <form id="recipeForm" enctype="multipart/form-data">
      <div class="recipe-form-group">
        <div class="recipe-image-upload" id="recipeImageUpload">
      ...
      <div class="recipe-form-group">
        ...
        <div class="ingredients-scroll-container" id="ingredientsContainer">
      ...
      <div class="recipe-form-group">
        ...
        ></textarea>
        <small style="color: #666; font-size: 12px;">Sabihin sa amin ang tungkol sa flavor, style, o type ng dish na hinahanap mo</small>
      </div>
      
      <!-- NEW: AI Image Generation Option -->
      <div class="recipe-form-group">
        <div class="option-checkbox">
          <input type="checkbox" id="generateImageCheckbox" name="generate_image" value="true" checked>
          <label for="generateImageCheckbox">Gumawa ng AI image ng suggested na dish</label>
        </div>
        <small style="color: #666; font-size: 12px;">Gagawa ang AI ng image ng recipe na isusuggest base sa iyong ingredients</small>
      </div>
...
<div class="container">
  ...
  <main class="chat-card">
    <div class="chat-header">
      ...
      <button class="history-btn" id="historyBtn" title="Chat History">
 ...
 <div class="messages" id="messages">
        ...
        <?php foreach ($userHistory as $item): ?>
          ...
          <div class="msg ai">
            ...
            <div class="bubble has-image">
                <!-- FIXED: Apply the same formatting to stored responses -->
                    ...
                    <?php if (strpos($item['message'], 'Generate image:') === 0 || strpos($item['message'], 'Generated recipe from ingredients:') === 0): ?>
                        <div class="image-generation-container">
                            <?php if (strpos($item['message'], 'Generated recipe from ingredients:') === 0): ?>
                                <div class="image-prompt"><strong>Recipe Image:</strong> Base sa iyong ingredients at description</div>
                            <?php else: ?>
      ...
      <?php else: ?>
        ...
        <div class="intro-container">
          ...
          <div class="intro-text">Kamusta <?php echo htmlspecialchars($username); ?>!</div>
          <div class="intro-subtext">Ano ang gusto mong lutuin ngayon?</div>
        </div>
      <?php endif; ?>
    </div>
    <!-- UPLOADED IMAGE PREVIEW -->
    ...
    <div id="image-options" class="image-options" style="display: none;">
      <div class="option-checkbox">
        ...
        <label for="generate-description">Gumawa ng description para sa image</label>
      </div>
    </div>

    <div class="chat-input-container" id="chat-input-container">
      <!-- Generate Recipe Button removed from quick tags (moved to plus dropdown) -->
        <!-- Generate Recipe button removed from here -->
      </div>

      <div class="composer">
        <div class="input">
          <textarea id="inputBox" placeholder="Ask Chef-AI about Filipino cuisine" rows="1"></textarea>
          ...
          <div class="plus-button-container">
            ...
            <div class="plus-dropdown" id="plus-dropdown">
              ...
              <!-- NEW: Generate Recipe option added to plus dropdown -->
              <div class="plus-dropdown-item generate-recipe-option" id="generate-recipe-option">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const quickTags = document.querySelectorAll('.tag-btn[data-sample]');
    ...
    if (historyBtn && mainSidebar) {
        console.log('Initializing mobile sidebar functionality');
        
        // Toggle sidebar
        historyBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            console.log('History button clicked - Opening sidebar');
        ...
        document.addEventListener('click', function(event) {
            if (mainSidebar.classList.contains('active') && 
                ...
                console.log('Closing sidebar - Click outside');
                closeSidebar();
            }
        });
        ...
        document.addEventListener('keydown', function(event) {
            if (event.key === 'Escape' && mainSidebar.classList.contains('active')) {
    ...
    // Generate Recipe option from plus dropdown
    const generateRecipeOption = document.getElementById('generate-recipe-option');
    
    // Verification error modal variables
    ...
    function updateNewChatButtonState() {
        if (!btnNew || !messages) return;
        
        const hasConversations = messages.querySelectorAll('.msg').length > 0;
    ...
    window.addEventListener('click', function(event) {
        ...
        if (event.target === verificationErrorModal) {
    ...
    if (uploadImageOption) {
        uploadImageOption.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Upload image option clicked');
            if (imageUpload) {
                imageUpload.click();
            }
            if (plusDropdown) {
    ...
    if (generateImageOption) {
        generateImageOption.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Generate image option clicked');
            currentMode = 'generate_image';
            if (inputBox) {
                inputBox.placeholder = "Ilarawan ang image na gusto mong gawin...";
            }
            if (imageOptions) {
    ...
    // Generate recipe option
    if (generateRecipeOption) {
        generateRecipeOption.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Generate recipe option clicked');
            if (recipeModal) {
    ...
    function clearUploadedImage() {
        ...
        if (inputBox) {
            inputBox.placeholder = "Ask ChefAI about Filipino cuisine";
        }
    }
        ...
        imageUpload.addEventListener('change', function(e) {
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    if (inputBox) {
                        inputBox.placeholder = "Ask about the image or leave blank for general analysis...";
                    }
                };
    ...
    function addIngredientField(ingredient = '') {
        ...
        ingredientInput.innerHTML = `
            <input type="text" name="ingredients[]" placeholder="Enter ingredient" value="${ingredient}" required>
        ...
        cookingLoader.innerHTML = `
            ...
            <div class="bubble">
                <div class="cooking-loader">
                    ...
                    <div class="loading-text">Sinusuri ang iyong larawan...</div>
                </div>
            </div>
        `;
        messages.appendChild(cookingLoader);
        ...
        cookingLoader.innerHTML = `
            ...
            <div class="bubble">
                <div class="cooking-loader">
                    ...
                    <div class="loading-text">Gumagawa ng larawan...</div>
                </div>
            </div>
        `;
        messages.appendChild(cookingLoader);
    ...
    async function handleSend() {
        ...
        if (hasUploadedImage && !message && currentMode === 'text') {
            sendImageAnalysis();
            return;
        }
        ...
        if (message === '' && !hasUploadedImage && currentMode !== 'generate_image') {
    ...
    async function sendImageAnalysis() {
        ...
        createMessage('user', userQuestion || 'Analyze this image', true, imageDataUrl);
        ...
        try {
            ...
            document.getElementById('image-analysis-loading')?.remove();
    ...
    async function sendImageGeneration(prompt, generateDescription = false) {
        ...
        if (generateDescriptionCheckbox) {
            generateDescriptionCheckbox.checked = false;
        }
    ...
    function typeEffect(text, historyId = null) {
        return new Promise(resolve => {
            ...
            function step() {
                ...
                if (bubble) {
                    bubble.innerHTML = formatResponse(partialText);
                }
                scrollToBottom();
...
document.addEventListener('submit', function(e) {
    // Handle bookmark forms with AJAX
    if (e.target && e.target.classList.contains('bookmark-form')) {
        e.preventDefault();
        const form = e.target;
    ...
    // Handle save image forms with AJAX
    if (e.target && e.target.classList.contains('save-image-form')) {
        e.preventDefault();
        const form = e.target;
        const formData = new FormData(form);
        const button = form.querySelector('.save-btn');
        const historyId = form.querySelector('input[name="save_image"]').value;
        
        if (historyId === '0') {
            alert('Please wait a moment for the image to save before using the save button.');
            return;
        }
...
if (inputBox) {
    ...
    if (quickTags.length > 0) {
        quickTags.forEach(btn => {
            btn.addEventListener('click', () => {
                const p = btn.getAttribute('data-sample');
                if (introContainer) {
                    introContainer.style.display = 'none';
                }
                if (chatInputContainer) {
    ...
    // Auto-refresh chat when page becomes visible
    document.addEventListener('visibilitychange', function() {
        if (!document.hidden) {
            // Page became visible
            console.log('Page visible - you could refresh chat here if needed');
        }
    });
...
function handleChatImageError(img) {
    ...
    // Optional: Update profile image in database

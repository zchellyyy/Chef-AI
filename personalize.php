...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ENHANCED MODAL STYLES - ISOLATED FROM SIDE NAV */
        .modal {
            display: none;
            position: fixed;
            z-index: 9999;
        ...
        /* Step Form - IMPROVED STEP INDICATOR */
        .step-indicator {
            display: flex;
            justify-content: center;
            align-items: center;
        ...
        .step.active {
            ...
            color: white;
            transform: scale(1.1);
            box-shadow: 0 0 0 4px rgba(108, 78, 49, 0.2);
        }
        ...
        /* NEW: Meal Time Selection Styles */
        .meal-time-selection {
            margin-bottom: 20px;
        }
        ...
        /* AI Bookmarked Recipe Modal Styles */
        .ai-recipe-details {
            margin-bottom: 20px;
        }
        
        .ai-recipe-details h3 {
            color: #6C4E31;
            margin-bottom: 10px;
            font-size: 1.2rem;
        }
        
        .ai-recipe-details ul {
            list-style-type: none;
            padding-left: 0;
        }
        
        .ai-recipe-details li {
            margin-bottom: 8px;
            padding-left: 20px;
            position: relative;
        }
        
        .ai-recipe-details li:before {
            content: "•";
            color: #C1856D;
            font-weight: bold;
            position: absolute;
        ...
        .ai-image-description h3 {
            color: #6C4E31;
            margin-bottom: 10px;
        }
        
        .ai-image-description p {
            line-height: 1.6;
            margin-bottom: 10px;
        }
        
        .ai-image-description b {
            color: #6C4E31;
            font-weight: 600;
        }
        
        /* Upload Recipe Modal Styles */
        .recipe-details-grid {
            display: grid;
...
<body>
            ...
            <?php if (!empty($bookmarkedRecipes)): ?>
                <div class="grid">
                    <?php foreach ($current_bookmarks as $item): 
                        ...
                        $truncated_message = strlen($message) > 100 ? substr($message, 0, 100) . '...' : $message;
                        
                        $response_preview = !empty($item['response']) ? 
                            (strlen($item['response']) > 100 ? substr($item['response'], 0, 100) . '...' : $item['response']) : 
                            'No response content';
                    ...
                    ?>
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-meta">
                                    <span><i class="fas fa-calendar"></i> <?php echo date('M j, Y g:i A', strtotime($item['created_at'])); ?></span>
                                </div>
                ...
                <div class="grid">
                    <?php foreach ($current_images as $image): 
                        $description = !empty($image['description']) ? 
                            (strlen($image['description']) > 100 ? substr($image['description'], 0, 100) . '...' : $image['description']) : 
                    ...
                    ?>
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-meta">
                                    <span><i class="fas fa-calendar"></i> <?php echo date('M j, Y g:i A', strtotime($image['saved_at'])); ?></span>
                                </div>
                                <div class="card-content">
        ...
        <div class="section">
            <div class="section-header">
                ...
                <button class="btn" onclick="openModal('mealPlannerModal')">
                    <i class="fas fa-plus"></i> Add Meal Planner
                </button>
            </div>
    ...
    <div id="recipeUploadModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
               <form id="uploadRecipeForm" action="upload_recipe.php" method="POST" enctype="multipart/form-data">
                    <div class="form-row">
                ...
                <div class="step-indicator">
                    <div class="step-container">
                        ...
                        <div class="step" data-step="2">
                            <span>2</span>
                            <div class="step-label">Plan Type</div>
                        </div>
                        <div class="step-line"></div>
                ...
                <form id="mealPlannerForm" method="POST">
                    ...
                    <div class="form-step active" id="step1">
                        ...
                        <div class="form-group meal-time-selection">
                            ...
                            <div class="meal-time-options">
                                ...
                                <span class="meal-time-select-all" id="selectAllMealTimes">Select All</span>
                            </div>
                        </div>
                    ...
                    <div class="form-step" id="step2">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="food_restrictions" rows="3" placeholder="List any allergies, religious restrictions, or dietary limitations (e.g., No pork, Allergic to shrimp, Halal only)"></textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <small style="color: #666;">Optional: Enter your estimated budget for meal planning</small>
                        </div>
                ...
                <form id="editMealPlanForm" method="POST">
                    ...
                    <div class="form-group meal-time-selection">
                        ...
                        <div class="meal-time-options" id="editMealTimeOptions">
                            ...
                            <span class="meal-time-select-all" id="editSelectAllMealTimes">Select All</span>
                        </div>
                    </div>
                    ...
                    <div class="form-group" id="editCustomMealOptions">
                        <label class="form-label">Your Custom Meals (5-10 recipes)</label>
                        <textarea class="form-control" name="edit_custom_meals" id="editCustomMeals" rows="5" placeholder="List your preferred meals, one per line (e.g., Menudo, Adobo, Sinigang)"></textarea>
                        <small style="color: #666;">Enter at least 5 meals for a balanced plan</small>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Food Restrictions</label>
                        <textarea class="form-control" name="edit_food_restrictions" id="editFoodRestrictions" rows="3" placeholder="List any allergies, religious restrictions, or dietary limitations (e.g., No pork, Allergic to shrimp, Halal only)"></textarea>
                    </div>
    ...
    <!-- AI Bookmarked Recipe Modal -->
    <div id="aiBookmarkedRecipeModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title"><i class="fas fa-star"></i> AI Bookmarked Recipe</h3>
                <button class="close" onclick="closeModal('aiBookmarkedRecipeModal')">&times;</button>
            </div>
            <div class="modal-body" id="aiBookmarkedRecipeBody">
                <!-- Content will be loaded here -->
            </div>
        </div>
    </div>
...
<script>
    ...
    // NEW: Meal Time Selection - Select All functionality
    ...
    // NEW: Edit Meal Time Selection - Select All functionality
    ...
    // Multi-step form functionality with improved step indicators
    ...
    function updateStepIndicators(fromStep, toStep, forward = true) {
        ...
        if (forward) {
            ...
            steps[toStep - 1]?.classList.remove('completed');
            
            if (toStep > 1) {
                stepLines[toStep - 2]?.classList.add('active');
            }
        } else {
            // Moving backward
            for (let i = toStep; i < steps.length; i++) {
                ...
                if (i < stepLines.length) {
                    stepLines[i]?.classList.remove('completed', 'active');
                }
            }
            ...
            if (toStep > 1) {
                stepLines[toStep - 2]?.classList.add('active');
                stepLines[toStep - 2]?.classList.remove('completed');
            }
        }
    }
    ...
    // Generate AI analysis - FIXED MEAL DISTRIBUTION LOGIC
    // FIXED generateAnalysis() function - with proper ingredient formatting
function generateAnalysis() {
    ...
    // Build dynamic prompt based on user inputs - WITH PROPER INGREDIENT FORMATTING
...
- Meals to include: ${selectedMealTimes.join(', ')}`;
    ...
    if (planType === 'custom' && customMealsArray.length > 0) {
        ...
        // FIXED: Correct meal distribution logic
        if (daysDiff === 1) {
            // Single day - BUFFET STYLE: Show all meals in ONE meal time entry
            ...
            prompt += `\n- Do NOT create multiple entries for the same meal time.`;
        } else {
            // Multiple days - DISTRIBUTE EVENLY across days
            ...
            prompt += `\n- Use ALL meals provided. Do not skip or add any meals.`;
        }
    } else if (planType === 'ai') {
        ...
        // For AI-generated plans with multiple days
        if (daysDiff === 1) {
            prompt += `\n- This is a single-day plan. Provide ${selectedMealTimes.length} balanced meals with variety.`;
        } else {
            prompt += `\n- This is a ${daysDiff}-day plan. Ensure meal variety across days and consider nutritional balance.`;
        }
    }
    ...
    if (formData.get('food_restrictions')) {
        ...
        prompt += `\n- IMPORTANT: All meals must strictly comply with these restrictions.`;
    }
    
    // Add specific instructions based on duration and budget
    if (daysDiff === 1) {
        prompt += `\n- This is a single-day meal plan for ${pax} people.`;
        prompt += `\n- CRITICAL: For single-day plans, create ONLY ONE entry for each selected meal time.`;
        prompt += `\n- DO NOT repeat the same meal time multiple times in the same day.`;
    } else if (daysDiff === 2) {
        prompt += `\n- This is a 2-day meal plan for ${pax} people. Ensure variety across days.`;
    } else if (daysDiff === 3) {
        prompt += `\n- This is a 3-day meal plan for ${pax} people. Create a balanced rotation of meals.`;
    } else {
        prompt += `\n- This is a ${daysDiff}-day meal plan for ${pax} people. Ensure meal variety.`;
    }
    ...
    // CRITICAL FORMATTING INSTRUCTIONS FOR UNIFORM INGREDIENT LISTING
...
2. INGREDIENT FORMATTING:
   ...
   - Example: "Chicken: 25 kg, cut into pieces"
   - Do NOT use symbols like •, *, or numbers before ingredients
...
4. EXAMPLE OF CORRECT FORMAT:
   1. Chicken Adobo
   - Chicken: 25 kg, cut into serving pieces
   - Soy Sauce: 6.25 cups (about 1.5 liters)
   - Vinegar: 6.25 cups (about 1.5 liters)
   - Garlic: 25 heads, crushed
   - Bay Leaves: 50-75 pieces
   - Whole Peppercorns: 25 teaspoons
   - Cooking Oil: 50 tablespoons (about 3 cups)
   - Sugar or Honey: To taste (estimate 1-2 cups)

   2. Pork Sinigang
   - Pork: 25 kg
   - Sinigang Mix: 25 packets or 50-75 tablespoons
   - Kangkong: 25 bunches
   - Sitaw: 12.5 cups, sliced
   - Radish: 25-50 pieces, sliced
   - Tomatoes: 50-75 pieces, quartered
   - Onions: 25 pieces, quartered
   - Long Green Chili: 50-75 pieces
   - Fish Sauce: To taste (at least 1-2 bottles)

5. DO NOT USE THIS WRONG FORMAT:
   (White or Cane): 6.25 cups (or about 1.5 liters), • Bawang: 25 ulo, durugin, • Dahon ng Laurel: 50–75 piraso, • Parninta: 25 teaspoons, buo, • Mantika: 50 tablespoons (or about 3 cups), • Asukal or Honey: To taste (estimate around 1–2 cups)
   
   This is WRONG because:
   - Uses parentheses incorrectly
   - Mixes languages (English and Tagalog)
   - Uses bullet symbols (•) instead of hyphens (-)
...
<b>🛒 SHOPPING LIST:</b>
[Organize by category with CLEAN, UNIFORM formatting as shown above]

<b>📦 Pantry Items:</b>
- Item 1: Quantity needed for ${pax} people
- Item 2: Quantity needed for ${pax} people
- Item 3: Quantity needed for ${pax} people
...
1. Current price trends for key ingredients
2. Estimated cost breakdown per category
3. Where to buy recommendations (wet markets, supermarkets, etc.)
4. Seasonal availability considerations
5. Cost-saving tips]
...
// UPDATED generateDayHeaders() function for better formatting
function generateDayHeaders(daysDiff, startDate, mealTimes, customMeals) {
    ...
    for (let i = 1; i <= daysDiff; i++) {
        ...
        // For single-day custom plans: list all meals in ONE entry per meal time
        if (daysDiff === 1 && isCustomPlan) {
            // SINGLE DAY: All meals listed with proper numbering
            ...
            customMeals.forEach((meal, index) => {
                ...
                headers += `- Prep Time: [Total time for all meals]\n`;
            ...
            mealTimes.forEach(mealTime => {
                ...
                headers += `- Prep Time: [Estimated minutes]\n`;
                headers += `- Estimated Cost: ₱[amount]\n`;
            });
        } else {
            // AI-GENERATED or no custom meals
            mealTimes.forEach(mealTime => {
                headers += `\n${mealTime.toUpperCase()}:\n`;
                headers += `[Meal Name]\n`;
...
// UPDATED formatAIAnalysis() to clean up messy ingredient formatting
function formatAIAnalysis(text) {
    ...
    // Clean up messy ingredient formatting
    formatted = formatted.replace(/•/g, '-'); // Replace bullet symbols with hyphens
    formatted = formatted.replace(/\(White or Cane\)/g, 'Vinegar'); // Fix specific cases
    formatted = formatted.replace(/Bawang/g, 'Garlic');
    formatted = formatted.replace(/Dahon ng Laurel/g, 'Bay Leaves');
    formatted = formatted.replace(/Parninta/g, 'Peppercorns');
    formatted = formatted.replace(/Mantika/g, 'Cooking Oil');
    formatted = formatted.replace(/Asukal/g, 'Sugar');
    ...
    formatted = formatted.replace(/(\d+)\.\s+([A-Za-z\s]+):/g, '<b>$1. $2:</b>');
    
    // Format headings
    formatted = formatted.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
    formatted = formatted.replace(/\n/g, '<br>');
    
    // Format ingredient lists with proper indentation
    formatted = formatted.replace(/^- (.*?)(<br>|$)/g, '&nbsp;&nbsp;&nbsp;&nbsp;- $1<br>');
    
    // Format shopping list categories
    formatted = formatted.replace(/(🛒|📦|🥬|🥩|🥛|💰|💡|⏰) (.*?):/g, '<b>$1 $2:</b>');
    
    return formatted;
}
    // Helper function to generate day headers with selected meal times - UPDATED TO PREVENT DUPLICATION
   function generateDayHeaders(daysDiff, startDate, mealTimes, customMeals) {
    let headers = '';
    const isCustomPlan = customMeals && customMeals.length > 0;
    ...
    function fetchAIResponse(prompt) {
        const apiKey = 'AIzaSyBLgKBfTIRzVFn-aj0riJjIMHubHKepYRs'; // Replace with your actual API key
        ...
        .catch(error => {
            ...
            // Fallback to a more basic response if API fails
            const form = document.getElementById('mealPlannerForm');
            const formData = new FormData(form);
            const planType = formData.get('plan_type');
            const daysDiff = 1; // Default to 1 day for fallback
            const selectedMealTimes = ['Breakfast', 'Lunch', 'Dinner'];
            
            let fallbackResponse = `
<b>📋 MEAL PLAN OVERVIEW:</b>
This is a personalized meal plan created based on your requirements. ${planType === 'custom' ? 'It uses your preferred meals' : 'AI has suggested balanced meals'} for your ${getDurationText(formData.get('duration_type'))} plan.

<b>📅 DAILY MEAL BREAKDOWN:</b>
[Day 1 - ${formData.get('start_date')}]
- Lunch: Buffet Lunch with Multiple Dishes
  - Description: A variety of dishes served buffet-style for all guests
  - Key Ingredients: Various proteins, vegetables, rice, seasonings
...
- Vegetable prices vary by season
- Eggs: ₱8-10 each

<b>Cost Breakdown:</b>
1. Pantry items: ₱300-400
2. Fresh produce: ₱200-300
3. Meat/protein: ₱500-700
4. Dairy: ₱100-150

<b>Where to Buy:</b>
- Wet markets: Best for fresh produce and meat
- Supermarkets: Better for pantry items and packaged goods
- Local talipapa: Good for smaller quantities

<b>Seasonal Considerations:</b>
- Vegetables are cheaper during rainy season
- Meat prices may fluctuate during holidays
- Stock up on non-perishables when on sale

<b>Cost-Saving Tips:</b>
1. Buy in bulk for frequently used items
2. Visit markets early for best selection
3. Consider alternative protein sources
4. Plan meals around seasonal produce

<b>💡 PREPARATION TIPS:</b>
1. Prepare ingredients in advance to save time
2. Cook rice in larger batches for multiple meals
3. Store leftovers properly for next day's meals
...
<b>💰 TOTAL ESTIMATED COST:</b> ₱${formData.get('pax') * 200} for ${formData.get('pax')} people`;
            ...
            alert('Note: Using fallback meal plan. AI service is currently unavailable. For best results, ensure your API key is valid.');
        });
    }
    ...
    // Close expanded analysis with Escape key
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && document.getElementById('analysisOverlay').classList.contains('active')) {
            closeExpandedAnalysis();
        }
    });
    ...
    function viewBookmarkedAI(historyId) {
        fetch('get_bookmarked_ai.php?id=' + historyId)
            .then(response => {
                if (!response.ok) {
    ...
    function viewSavedImage(historyId) {
        fetch('get_saved_image.php?id=' + historyId)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
    ...
    function viewSavedRecipe(recipeId) {
        fetch('get_saved_recipe.php?id=' + recipeId)
    ...
    function viewUploadedRecipe(recipeId) {
        fetch('get_uploaded_recipe.php?id=' + recipeId)
            .then(response => response.json())
            .then(data => {
    ...
    function deleteUploadedRecipe(recipeId) {
        if (confirm('Are you sure you want to delete this uploaded recipe?')) {
            fetch('delete_uploaded_recipe.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
    ...
    function viewMealPlan(planId) {
        fetch('get_meal_plan.php?id=' + planId)
            .then(response => response.json())
            .then(plan => {
    ...
    // Helper function to format AI recipe details with bullet points and bold text
